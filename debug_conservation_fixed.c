...
// Test simple de conservation avec algorithme de split corrigé
int test_conservation_simple_fixed(uint64_t input_a, uint64_t input_b) {
    ...
    // Split simple - algorithme corrigé
...
int main(void) {
    printf("=== DEBUG CONSERVATION FIXED ===\n");
    ...
    printf("Test simple:\n");
    if (test_conservation_simple_fixed(a, b) == 0) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        printf("Test %d:\n", i+1);
        if (test_conservation_simple_fixed(ra, rb) == 0) {
            printf("✅ Conservation OK\n");
            success_count++;
        } else {
            printf("❌ Conservation FAILED\n");
    ...
    printf("Résultat: %d/10 tests réussis (%.1f%%)\n", success_count, (double)success_count / 10 * 100.0);
