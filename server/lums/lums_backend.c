...
// Fonctions publiques simplifiées pour compatibilité API
...
int lums_compute_cycle(uint64_t lum_source, int cycle_count, uint64_t* result) {
    return lums_compute_cycle_real(lum_source, cycle_count, result);
}

double lums_compute_sqrt(double x) {
    return lums_compute_sqrt_via_lums(x);
}

bool lums_test_prime(uint64_t n) {
    return lums_test_prime_real(n);
}

int lums_store_memory(const char* slot_name, uint64_t lum_data) {
    return lums_store_memory_real(slot_name, lum_data);
}

int lums_retrieve_memory(const char* slot_name, uint64_t* lum_data) {
    return lums_retrieve_memory_real(slot_name, lum_data);
}
