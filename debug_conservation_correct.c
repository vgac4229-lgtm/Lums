...
// Test simple de conservation avec algorithme de split correct
int test_conservation_simple_correct(uint64_t input_a, uint64_t input_b) {
    ...
    // Split simple - algorithme correct
...
int main(void) {
    printf("=== DEBUG CONSERVATION CORRECT ===\n");
    ...
    printf("Test simple:\n");
    if (test_conservation_simple_correct(a, b) == 0) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        printf("Test %d:\n", i+1);
        if (test_conservation_simple_correct(ra, rb) == 0) {
            printf("✅ Conservation OK\n");
            success_count++;
        } else {
            printf("❌ Conservation FAILED\n");
